...
class Tmaze(object):
    ...
    def update_state(self, action):
        ...
        for c in range(len(self.advs)):
            ...
            # retrieve the valid adversary actions based off of its current location
            advs_valid_actions = self.advs_valid_actions(cell = self.advs[c],flag = self.flags[c])
            # randomly choose an adversary action from valid actions
            advs_action = random.choice(advs_valid_actions)
    ...
    def advs_valid_actions(self, cell=None, flag=None):
        ...
        # possible actions
        actions = [0, 1, 2, 3]
        # retrieves # of rows and columns of our maze
        nrows, ncols = self.maze.shape
        # code below ensures our adversary is always moving only in the cells
        # adjacent to the flag
        if advs_row > flag_row:
            actions.remove(3)
        elif advs_row < flag_row:
            actions.remove(1)

        if advs_col < flag_col:
            actions.remove(0)
        elif advs_col > flag_col:
            actions.remove(2)
